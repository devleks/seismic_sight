...
### Added
- **Camera Switching:** Added the ability to switch between front and back cameras on supported devices (mobile and tablet).
- **Smart Camera Default:** The application now automatically defaults to the back (environment) camera on mobile and tablet devices for a better scanning experience.

### Changed
- Updated documentation (`README.md`, `USERGUIDE.md`, `CHANGELOG.md`) and bumped version to `1.5.0`.

## [1.4.0] - 2026-03-14

### Added
- **API Key Obfuscation:** Added a security enhancement to obfuscate the Gemini API key stored in the browser's `sessionStorage`. This prevents the key from being displayed in clear text within browser developer tools, protecting it from casual shoulder-surfing.

### Changed
- Updated documentation (`README.md`, `USERGUIDE.md`, `CHANGELOG.md`) and bumped version to `1.4.0`.
